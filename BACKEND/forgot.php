...
// client_password
            ...
            try {
                ...
                $select = $con->query("SELECT client_fullname,client_email FROM $tbname WHERE client_email='$email'");
                $slt = $select->fetch();
                if ($slt && $slt["client_email"] == $email) {

                    $username = $slt["client_fullname"];
                    ...
                    $mail->SMTPSecure = "ssl";
                    $mail->Port = 465;
                    $mail->setFrom($usernamemailer, "BooKM");
